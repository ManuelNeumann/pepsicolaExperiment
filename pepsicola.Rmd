...
```{r, echo=FALSE}
tab1 <- table(simulation1$x)
proptab1 <- table(simulation1$x) %>% prop.table() %>% {.*100} %>% round(1) %>% paste0(., "%")
cproptab1 <- table(simulation1$x) %>% prop.table() %>% cumsum() %>% {.*100} %>% round(1) %>% paste0(., "%")

tabmatr1 <- rbind(tab1, proptab1, cproptab1)
dimnames(tabmatr1)[[1]] <- c("Verteilung", "Prozentuale Verteilung", "Aufsummierte Prozente")

kable(tabmatr1, align = "c")
```
...
```{r, echo = FALSE}
tab3 <- table(simulation2$x)
proptab3 <- table(simulation2$x) %>% prop.table() %>% {.*100} %>% round(2) %>% paste0(., "%")
cproptab3 <- table(simulation2$x) %>% prop.table() %>% cumsum() %>% {.*100} %>% round(2) %>% paste0(., "%")

tabmatr3 <- rbind(tab3, proptab3, cproptab3)
dimnames(tabmatr3)[[1]] <- c("Verteilung", "Prozentuale Verteilung", "Aufsummierte Prozente")

kable(tabmatr3, align = "c")
```
